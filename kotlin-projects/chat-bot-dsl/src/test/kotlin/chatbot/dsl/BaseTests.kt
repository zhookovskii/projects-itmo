...
class BaseTests {
    ...
    @Test
    fun predicateContext() {
        val helloText: MessageProcessorPredicate = { it.text.contains("hello") }
        val friendText: MessageProcessorPredicate = { it.text.contains("friend") }
        val momText: MessageProcessorPredicate = { it.text.contains("mom") }
        val bot = chatBot(testClient) {
            behaviour {
                (helloText * friendText).into {
                    onMessage {
                        client.sendMessage(message.chatId, "i'm not your friend")
                    }
                }
                ...
                helloText.into {
                    momText.into {
                        onMessage {
                            client.sendMessage(message.chatId, "i'm not your mom")
                        }
                    }
                    onMessage {
                        client.sendMessage(message.chatId, "goodbye!")
                    }
                }
            }
        }
        bot.sendText("talk to me")
        assertNoMessages()

        bot.sendText("hello!")
        assertMessage(1L, "goodbye!")

        bot.sendText("hello friend!")
        assertMessage(1L, "i'm not your friend")

        bot.sendText("howdy friend!")
        assertNoMessages()

        bot.sendText("hello mom!")
        assertMessage(1L, "i'm not your mom")
